...
void processMapPt(const geometry_msgs::Point &curr_pt, cv::Mat &occupied, cv::Mat &visited, 
	...
	if (use_height_thresholding){
		...
		//cout << "pt_pos_y is" << pt_pos_y << endl;
		...
		//cout << "transformed_point_height is" << transformed_point_height << endl;
		is_ground_pt = pt_pos_y < -2;         //地面上的点
		//is_toohigh_pt = pt_pos_y > 0.5;      //太高了
	}
#ifndef DISABLE_FLANN
...
#endif
	if (is_ground_pt || is_in_horizontal_plane || is_toohigh_pt) {
		++visited.at<float>(pt_pos_grid_z, pt_pos_grid_x);        //提高栅格的visit值
